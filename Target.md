...
* **功能点 B：高吞吐量串口通信与 DMA**
* **做法**: 既然没有外部传感器，ESP32 和 OrangePi 之间的数据吞吐量就要做大。使用 ESP32 的 UART 驱动，开启 **UART DMA** 模式。
* **协议设计**: 设计一个含帧头、长度、Payload、CRC16 校验的私有协议。
* *面试话术*: “为了解决高频采样下的数据堆积问题，我手写了基于 DMA 的 UART 驱动，并配合 FreeRTOS 的 StreamBuffer 实现了解析，避免了频繁中断带来的 CPU 开销。”

* **功能点 C：ESP-IDF 原生 OTA**
* ESP32 的分区表机制非常成熟。配置 Partition Table 为 `factory, app1, app2`，实现双分区 OTA。
* 固件可以通过串口从 OrangePi 传输过来写入。
...
* **网关程序 (C++/Python)**:
* 监听串口，接收 ESP32 发来的处理后的数据（如 FFT 分析结果）。
* 通过 MQTT 协议（使用 Mosquitto 库）将数据发送到阿里云/腾讯云 IoT 平台。
* **反向控制**: 云端下发指令 -> OrangePi 解析 -> 串口转发给 ESP32 -> ESP32 点亮 LED 或切换模式。
...
* OrangePi 安装 Linux 系统，配置好 EC20 上网。
* 编写 ESP32 的 UART DMA 发送代码。
* 编写 OrangePi 的 Python/C++ 脚本读取串口数据并打印。
* 打通 MQTT：ESP32 数据 -> 串口 -> OrangePi -> 4G -> 手机端查看。
...
> 3. **可靠通信协议**: 设计基于 **UART DMA** 的板间通信协议，包含帧同步与 CRC 校验，解决了 MCU 与 Linux 上位机之间的高速数据传输丢包问题。
